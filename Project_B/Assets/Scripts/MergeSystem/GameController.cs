...
public class GameController : MonoBehaviour
{
    ...
    // Slot id, Slot calss 관리하기 위한 자료 구조
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButton(0) && carryingItem)        // 잡고 이동 시킬 때
        {
            OnItemSelected();
        }

        if (Input.GetMouseButtonUp(0))  // 마우스 버튼 놓을 때
        {
            SendRayCast();
        }
    ...
    void SendRayCast()
    {
        ...
        if(Physics.Raycast(ray, out hit))
        {
            ...
            { // 빈 슬롯에 아이템을 배치
                slot.CreateItem(carryingItem.itemId);       // 잡고 있는 것 슬롯 위치에 생성
                Destroy(carryingItem);      // 잡고 있던 것 파괴
            }
    ...
    { // 병합 함수
        ...
        slot.CreateItem(carryingItem.itemId + 1);   // 슬롯에 있는 다음 번호 물체 생성
        Destroy(carryingItem.gameObject);           // 잡고 있는 물체 파괴
    }

    void OnItemSelected()
    {   // 아이템을 선택하고 마우스 위치로 이동
        _target = Camera.main.ScreenToWorldPoint(Input.mousePosition);  // 좌표 변환
        _target.z = 0;
